...
for(n.current in given.n)
{
  ...
  for(b in 1:given.n.samples)
  {
    ...
    cv.f.1 <- function(u) 
    { 
      cv.least.squares(xym.sample$x, xym.sample$y, k.func, u, W.matr[,1])
    }
    cv.f.2 <- function(u) 
    { 
      cv.least.squares(xym.sample$x, xym.sample$y, k.func, u, W.matr[,2])
    }
    ...
    # Підрахунок інтегральних оцінок в заданих точках
    festim.1 <- function(t)
    {
      loc.lin.est(
    ...
    estim.1 <- integrate(
      function(t) { (festim.1(t) - g.func.selected(1)(t))^2 },
      lower=0, upper=1, subdivisions = 1000, stop.on.error = FALSE
    )$value
    estim.2 <- integrate(
      function(t) { (festim.2(t) - g.func.selected(2)(t))^2 },
      lower=0, upper=1, subdivisions = 1000, stop.on.error = FALSE
    )$value
  ...
  print(mean(pointwise.results.1))
  
  print("Друга компонента:")
  print(mean(pointwise.results.2))
}